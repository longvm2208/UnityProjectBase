...
namespace ObjectPooling.SimplePool
{
    public static class SimplePool
    {
        ...
        private static Dictionary<PoolMember, Pool> poolOfWhichPrefabDict = new Dictionary<PoolMember, Pool>();
        private static Dictionary<PoolMember, Pool> memberOfWhichPoolDict = new Dictionary<PoolMember, Pool>();

        public static void Preload(PoolMember prefab, int amount, Transform parent)
        {
            if (!poolOfWhichPrefabDict.ContainsKey(prefab))
            {
                poolOfWhichPrefabDict.Add(prefab, new Pool(prefab, amount, parent));
            }
        }
        ...
        {
            if (!poolOfWhichPrefabDict.ContainsKey(prefab) || poolOfWhichPrefabDict[prefab] == null)
            {
                poolOfWhichPrefabDict.Add(prefab, new Pool(prefab, DEFAULT_AMOUNT, null));
            }

            PoolMember member = poolOfWhichPrefabDict[prefab].Spawn();
        ...
        public static PoolMember Spawn(PoolMember prefab, Transform parent)
        {
            if (!poolOfWhichPrefabDict.ContainsKey(prefab) || poolOfWhichPrefabDict[prefab] == null)
            {
                poolOfWhichPrefabDict.Add(prefab, new Pool(prefab, DEFAULT_AMOUNT, parent));
            }

            PoolMember member = poolOfWhichPrefabDict[prefab].Spawn(parent);
        ...
        public static PoolMember Spawn(PoolMember prefab, Transform parent, Vector3 localPosition)
        {
            ...
            PoolMember member = poolOfWhichPrefabDict[prefab].Spawn(parent, localPosition);
        ...
        public static PoolMember Spawn(PoolMember prefab, Vector3 position, Quaternion rotation)
        {
            ...
            PoolMember member = poolOfWhichPrefabDict[prefab].Spawn(position, rotation);
        ...
        {
            if (memberOfWhichPoolDict.ContainsKey(member))
            {
                memberOfWhichPoolDict[member].Despawn(member);
            }
            else
            {
        ...
        public class Pool
        {
            ...
            public Pool(PoolMember prefab, int amount, Transform parent)
            {
                ...
                for (int i = 0; i < amount; i++)
                {
                    ...
                    memberOfWhichPoolDict.Add(member, this);
                    pool.Enqueue(member);
